...
export function ScenarioBuilderMessageFeed({
    ...
    onRejectCommandExec,
    onInterruptCommandExec,
}: ScenarioBuilderMessageFeedProps) {
    ...
    return (
        <section
            ...
            className="h-full min-h-0 flex-1 space-y-4 overflow-y-auto rounded-2xl bg-main-900/55 p-2 ring-main-300/15"
        >
            {messages
                .filter((message) => message.author === "user")
                .map((message) => {
                    ...
                    const isActiveResponse = activeResponseToId === message.id;
