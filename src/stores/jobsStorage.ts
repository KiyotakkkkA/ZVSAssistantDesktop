...
class JobsStorage {
    ...
    async loadJobEvents(jobId: string): Promise<void> {
        ...
        const events = await jobsApi.getJobEvents(jobId);
    ...
    async createJob(payload: CreateJobPayload): Promise<JobRecord | null> {
        ...
        const createdJob = await jobsApi.createJob(payload);
    ...
    async cancelJob(jobId: string): Promise<boolean> {
        ...
        const isCancelled = await jobsApi.cancelJob(jobId);
    ...
    private ensureRealtimeSubscription(): void {
        ...
        this.unsubscribeFromEvents = jobsApi.onJobEvent((event) => {
